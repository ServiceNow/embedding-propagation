...
class PretrainWrapper(BaseWrapper):
    ...
    def train_on_batch(self, batch):
        ...
        if self.exp_dict["embedding_prop"] == True:
            one_hot_labels = torch.zeros(1, b, n, device=embeddings.device, dtype=torch.float)
            _, propagator = standarized_label_prop(embeddings.view(1, -1, c),
                                        one_hot_labels.view(1, -1, n))
            embeddings = _propagate(embeddings.view(1, -1, c), propagator).view(b, c)
